...
import {
	useAgentJobs,
	jobMatchesReviewContext,
} from "@plannotator/ui/hooks/useAgentJobs";
...
import { submitPlatformReviewTargets } from "./utils/platformReviewSubmission";
import {
...
const ReviewApp: React.FC = () => {
	...
	const [descriptionAnnotations, setDescriptionAnnotations] = useState<
		Annotation[]
	>([]);
	const [selectedDescriptionAnnotationId, setSelectedDescriptionAnnotationId] =
	...
	const [commentAnnotations, setCommentAnnotations] = useState<
		CommentAnnotation[]
	>([]);
	const [selectedCommentAnnotationId, setSelectedCommentAnnotationId] =
		useState<string | null>(null);
	...
	const registerAllFilesCollapseToggle = useCallback(
		(toggle: (() => void) | null) => {
	...
	const [allFilesVisibleFile, setAllFilesVisibleFile] = useState<string | null>(
		null,
	);
	...
	const [guideRevealFile, setGuideRevealFile] = useState<{
		path: string;
		token: number;
	} | null>(null);
	useEffect(() => {
	...
	const handleGuideRevealFile = useCallback((filePath: string) => {
		setGuideRevealFile((prev) => ({
			path: filePath,
			token: (prev?.token ?? 0) + 1,
		}));
	}, []);
	const [copyFeedback, setCopyFeedback] = useState<string | null>(null);
	const [copyRawDiffStatus, setCopyRawDiffStatus] = useState<
		"idle" | "success" | "error"
	>("idle");
	...
	const [workspaceDiffOptions, setWorkspaceDiffOptions] = useState<
		DiffOption[] | null
	>(null);
	// Two bases:
	...
	const [sessionPanelView, setSessionPanelView] = useState<
		"sections" | "commits" | "tree" | null
	>(null);
	const panelView: "sections" | "commits" | "tree" =
		sessionPanelView ?? persistedPanelView;
	const selectPanelView = useCallback(
		(view: "sections" | "commits" | "tree") => {
	...
	const [submitted, setSubmitted] = useState<
		"approved" | "feedback" | "exited" | false
	>(false);
	...
	const commitsCapable =
		!prMetadata && reviewMode !== "workspace" && gitContext?.vcsType === "git";
	const showCommitsPanel = commitsCapable && panelView === "commits";

	const prStackCallbacksRef = useRef<
		import("./hooks/usePRStack").PRStackCallbacks | null
	>(null);
	const {
		isSwitchingPRScope,
		isLoadingFullDiff,
	...
	const [showDestSpotlight, setShowDestSpotlight] = useState(
		needsDestinationSpotlight,
	);
	...
	const [platformActionError, setPlatformActionError] = useState<string | null>(
		null,
	);
	...
	const [platformRecoveryPersistsRefresh, setPlatformRecoveryPersistsRefresh] =
		useState(false);
	...
	} = useExternalAnnotations<CodeAnnotation>({ enabled: !!origin });
	const agentJobs = useAgentJobs({ enabled: !!origin && aiUIEnabled });

	// Persisted agent-review settings (engine + per-engine model/effort) shared
	// with the Settings AgentsTab surface. Read-only here — we don't mutate.
	// Reused by handleExplainCommit below to launch the explain-commit skill on
	// whatever engine the user already picked for reviews.
	const agentSettings = useAgentSettings();

	// Which commit's explain-commit skill run is currently in flight (null when
	// none). Drives the spinner on that CommitsPanel row + prevents double-fire.
	const [explainingCommitSha, setExplainingCommitSha] = useState<string | null>(
		null,
	);
	...
	const openDiffFile = useCallback(
		(filePath: string) => {
			...
			setActiveFileIndex(
				files.findIndex((candidate) => candidate.path === filePath),
			);
			needsInitialDiffPanel.current = false;
		},
		[dockApi, files],
	...
	// Only user-authored annotations flow OUTBOUND to the agent and platform PR
	// review posts. AI-generated annotations (any `source` set via
	// /api/external-annotations — e.g. the plannotator-explain-commit skill's
	// what/why/assumptions notes) STILL render in the UI through `allAnnotations`
	// so the reviewer sees them, but they are stripped here so Send Feedback /
	// Copy Feedback / platform PR comments contain only what the human wrote.
	// (editor / description / comment channels have no external ingestion path
	// today so no equivalent filter is applied there.)
	const userAnnotations = useMemo(
		() => allAnnotations.filter((a) => !a.source),
		[allAnnotations],
	);

	// Auto-save code annotation drafts
	const { draftBanner, restoreDraft, getDraftGeneration, dismissDraft } =
	...
	const handleRestoreDraft = useCallback(() => {
		...
		if (restored.descriptionAnnotations.length > 0)
			setDescriptionAnnotations(restored.descriptionAnnotations);
		if (restored.commentAnnotations.length > 0)
			setCommentAnnotations(restored.commentAnnotations);
		if (restored.viewedFiles.length > 0)
			setViewedFiles(new Set(restored.viewedFiles));
	}, [restoreDraft]);
	...
	>([]);
	const [aiDefaultProvider, setAiDefaultProvider] = useState<string | null>(
		null,
	);
	...
	const [showLookAndFeel, setShowLookAndFeel] = useState(
		needsLookAndFeelAnnouncement,
	);
	...
	const guideIntroVisible =
		aiUIEnabled && showGuideIntro && guideIntroEligibleRef.current === true;
	...
	const handleAskAIForFile = useCallback(
		(filePath: string, question: string) => {
	...
	const getAIHistoryForFile = useCallback(
		(filePath: string) => {
			if (!pendingSelection) return [];
	...
	useEffect(() => {
		...
		const handler = (e: KeyboardEvent) => {
			if (
				...
				e.preventDefault();
				setTourDialogJobId((prev) =>
					prev === DEMO_TOUR_ID ? null : DEMO_TOUR_ID,
				);
			}
		};
	...
	const { activeWorktreePath, activeDiffBase } = useMemo(() => {
		if (diffType.startsWith("worktree:")) {
			...
			if (
				commitIdx !== -1 &&
				/^commit:[0-9a-f]{4,64}$/i.test(rest.slice(commitIdx + 1))
			) {
				return {
					activeWorktreePath: rest.slice(0, commitIdx),
					activeDiffBase: rest.slice(commitIdx + 1),
				};
			}
	...
	const activeCommitContext = useMemo(() => {
		...
		return {
			sha,
			subject: commitInfo?.sha === sha ? commitInfo.subject : undefined,
		};
	...
	const canUseLiveWorkspaceActions =
	...
	const jobMatchesCurrentContext = useCallback(
		(job: AgentJobInfo) =>
			jobMatchesReviewContext(job, prMetadata?.url, activeWorktreePath),
	...
	const openSemanticDiffPanel = useCallback(
		(options?: { autoFallbackOnError?: boolean }) => {
			if (!dockApi) return;
			semanticDiffAutoFallbackPending.current =
				options?.autoFallbackOnError === true;
	...
	useEffect(() => {
		if (!dockApi || !needsInitialDiffPanel.current || files.length === 0)
			return;
		needsInitialDiffPanel.current = false;
		...
		fetch("/api/diff")
			...
			.then(
				(data: {
					...
					const apiFiles = orderFilesBySections(
						parseDiffToFiles(data.rawPatch),
						data.sections,
					);
					setDiffData({
						files: apiFiles,
					...
					setWorkspaceDiffOptions(
						data.mode === "workspace" ? (data.diffOptions ?? []) : null,
					);
					...
					updatePRSession({
						...
						...(data.prStackInfo !== undefined && {
							prStackInfo: data.prStackInfo,
						}),
						...(data.prStackTree !== undefined && {
							prStackTree: data.prStackTree,
						}),
						...(data.prDiffScope && { prDiffScope: data.prDiffScope }),
						...(data.prDiffScopeOptions && {
							prDiffScopeOptions: data.prDiffScopeOptions,
						}),
						...(data.prMetadata && {
	...
	const allFilesOrder: "tree" | "list" =
		sectionsAvailable && panelView === "sections" ? "list" : "tree";
	...
	const isPathStageable = useCallback(
	...
	useEffect(() => {
		const handler = (e: KeyboardEvent) => {
			if (e.metaKey || e.ctrlKey || e.shiftKey || isTypingTarget(e.target))
				return;
	...
	function applyPRResponse(
		...
		if (isPRSwitch) {
			...
			const preserved = currentFile
				? nextFiles.findIndex((f) => f.path === currentFile.path)
				: -1;
	...
	const fetchDiffSwitch = useCallback(
		async (
			fullDiffType: string,
			baseOverride?: string,
			options?: { preserveFile?: boolean; explicitBase?: boolean },
		): Promise<boolean> => {
			...
			try {
				const res = await fetch("/api/diff/switch", {
					...
					body: JSON.stringify({
						...
						...((baseOverride ?? selectedBase) && {
							base: baseOverride ?? selectedBase,
						}),
				...
				const nextFiles = orderFilesBySections(
					parseDiffToFiles(data.rawPatch),
					data.sections,
				);
				applySemanticDiffAdvert(data.semanticDiff);
				...
				if (options?.preserveFile) {
					...
					const nextIdx = currentPath
						? nextFiles.findIndex((f) => f.path === currentPath)
						: -1;
					if (nextIdx !== -1) {
	...
	const handleDiffSwitch = useCallback(
		async (baseDiffType: string) => {
			...
			const enteringEvolog =
				baseDiffType === "jj-evolog" &&
				gitContext?.jjEvologs &&
				gitContext.jjEvologs.length >= 2;
			const leavingEvolog =
				!enteringEvolog &&
				activeDiffBase === "jj-evolog" &&
	...
	const handlePanelViewSelect = useCallback(
		(view: "sections" | "commits" | "tree") => {
			if (view === "commits") {
	...
	// AI-explain a specific commit: switch the diff onto that commit (so the
	// review server's aiReviewContext = that commit's diff-vs-first-parent) and
	// launch a review job with the plannotator-explain-commit skill. The
	// resulting annotations post to /api/external-annotations with a
	// per-job source, so the userAnnotations filter above naturally excludes
	// them from outbound feedback.
	const handleExplainCommit = useCallback(
		async (sha: string) => {
			if (explainingCommitSha) return; // one at a time
			setExplainingCommitSha(sha);
			try {
				// Land the diff on this commit first. If we're already there, skip
				// the switch (fetchDiffSwitch returns true on success / current).
				const fullDiffType = activeWorktreePath
					? `worktree:${activeWorktreePath}:commit:${sha}`
					: `commit:${sha}`;
				if (fullDiffType !== diffType) {
					const switched = await fetchDiffSwitch(fullDiffType);
					if (!switched) {
						toast("Could not switch to that commit", {
							description: "Diff switch failed — explain aborted.",
							position: "top-right",
						});
						return;
					}
				...
				// Build a launch payload that mirrors AgentsTab's buildReviewLaunch,
				// but with reviewProfileId pinned to the explain-commit skill.
				// The engine is whatever the user already picked for reviews — falls
				// through to a codex/claude default if their pick isn't available at
				// runtime (agentJobs.launchJob will surface the specific 400).
				const engine = agentSettings.reviewEngine;
				const reviewProfileId = "skill:plannotator-explain-commit";
				const shortSha = sha.slice(0, 7);
				const label = `Explain ${shortSha}`;
				type LP = Parameters<typeof agentJobs.launchJob>[0];
				const launch: LP =
					engine === "claude"
						? {
								provider: "claude",
								label,
								model: agentSettings.claudeModel,
								effort: agentSettings.claudeEffort,
								reviewProfileId,
							}
						: engine === "cursor"
							? {
									provider: "cursor",
									label,
									...(agentSettings.cursorModel &&
									agentSettings.cursorModel.toLowerCase() !== "auto"
										? { model: agentSettings.cursorModel }
										: {}),
									reviewProfileId,
								}
							: engine === "opencode"
								? {
										provider: "opencode",
										label,
										...(agentSettings.opencodeModel
											? { model: agentSettings.opencodeModel }
											: {}),
										reviewProfileId,
									}
								: engine === "pi"
									? {
											...
											...(agentSettings.piModel
												? { model: agentSettings.piModel }
												: {}),
											thinking: agentSettings.piThinking,
											reviewProfileId,
										}
									: engine === "copilot"
										? {
												provider: "copilot",
												label,
												...(agentSettings.copilotModel
													? { model: agentSettings.copilotModel }
													: {}),
												reviewProfileId,
											}
										: {
												provider: "codex",
												label,
												model: agentSettings.codexModel,
												reasoningEffort: agentSettings.codexReasoning,
												...(agentSettings.codexFast && { fastMode: true }),
												reviewProfileId,
											};

				try {
					await agentJobs.launchJob(launch);
				} catch (err) {
					const msg = err instanceof Error ? err.message : String(err);
					// A missing/uncurated skill returns 400 with a specific message.
					// Surface it verbatim plus a hint about how to enable it, so a
					// first-time user knows what to do next.
					toast("Explain commit failed", {
						description:
							msg +
							" — enable plannotator-explain-commit in Settings → AI Reviews, or install it globally (see the skill's README).",
						position: "top-right",
						duration: 6000,
					});
				}
	...
	const handleFetchBase = useCallback(async () => {
		...
		try {
			...
			if (
				now.diffType === captured.diffType &&
				now.selectedBase === captured.selectedBase
			) {
				await fetchDiffSwitch(
					captured.diffType,
					captured.selectedBase ?? undefined,
					{ preserveFile: true },
				);
			}
		} catch {
	...
	const handleAddDescriptionAnnotation = useCallback(
		(ann: Annotation) => {
			// Stamp the active PR so the note stays bound to it across an in-place switch.
			setDescriptionAnnotations((prev) => [
				...prev,
				{ ...ann, prUrl: prMetadata?.url },
			]);
	...
	const handleSelectDescriptionAnnotation = useCallback(
		(id: string | null) => {
	...
	const handleAskAIForDescription = useCallback<CommentAskAIHandler>(
		(question, context) => {
			askAI({
				prompt: question,
				scope: {
					kind: "selection",
					label: context.label ?? "PR description",
					text: context.text,
				},
			});
		},
	...
	const handleAddCommentAnnotation = useCallback(
		(
			commentId: string,
	...
	const handleSelectCommentAnnotation = useCallback(
		(id: string | null) => {
	...
	const handleAskAIForComment = useCallback<CommentAskAIHandler>(
		(question, context) => {
			askAI({
				prompt: question,
				scope: {
					kind: "selection",
					label: context.label ?? "PR comment",
					text: context.text,
				},
			});
		},
	...
	const visibleDescriptionAnnotations = useMemo(
		() =>
			descriptionAnnotations.filter((a) =>
				proseAnnotationMatchesPr(a, prMetadata?.url),
			),
		[descriptionAnnotations, prMetadata?.url],
	);
	const visibleCommentAnnotations = useMemo(
		() =>
			commentAnnotations.filter((a) =>
				proseAnnotationMatchesPr(a, prMetadata?.url),
			),
	...
	const handleNavigateToAnnotation = useCallback(
		(id: string | null) => {
			...
			if (guideOpen) {
				...
				const fileIndex = files.findIndex(
					(f) => f.path === annotation.filePath,
				);
				if (fileIndex !== -1) handleFileSwitch(fileIndex);
			}
			setSelectedAnnotationId(id);
			setScrollTargetAnnotation((prev) => ({
				id,
				token: (prev?.token ?? 0) + 1,
			}));
		},
	...
	const reviewStateValue = useMemo<ReviewState>(
		() => ({
			...
			activeSearchMatchId,
			activeSearchMatch:
				activeSearchMatch?.filePath === files[activeFileIndex]?.path
					? activeSearchMatch
					: null,
			searchMatches,
			...
			onCodeNavRequest: canUseLiveWorkspaceActions
		...
		[
			...
			handleSelectDescriptionAnnotation,
			handleDeleteDescriptionAnnotation,
			handleAskAIForDescription,
			visibleCommentAnnotations,
			selectedCommentAnnotationId,
			handleAddCommentAnnotation,
			handleSelectCommentAnnotation,
			handleDeleteCommentAnnotation,
			handleAskAIForComment,
			commentScrollTarget,
			selectedAnnotationId,
			scrollTargetAnnotation,
			pendingSelection,
	...
	const feedbackMarkdown = useMemo(() => {
		...
		// userAnnotations — not allAnnotations — so AI-generated notes never leak
		// into what gets sent back to the coding agent or copied to the clipboard.
		const parts: string[] = [];
		...
		const prose = buildProseFeedback(
			visibleDescriptionAnnotations,
			visibleCommentAnnotations,
			prContext?.body,
		);
	...
	const totalAnnotationCount =
		...
		visibleEditorAnnotations.length +
		visibleDescriptionAnnotations.length +
		visibleCommentAnnotations.length;
	...
	const handleExit = useCallback(async () => {
		...
		try {
			const res = await fetch(
				`/api/exit?draftGeneration=${getDraftGeneration()}`,
				{ method: "POST" },
			);
			if (res.ok) {
				setSubmitted("exited");
		...
		async (
			...
			try {
				...
				if (plan.targets.some((target) => target.status === "blocked")) {
					throw new Error(
						`Automatic retry is blocked until the ${mrLabel} is inspected.`,
					);
				}
				...
				if (
					action === "approve" ||
					(targets.length === 0 && generalComment?.trim())
				) {
					const currentTarget = plan.targets.find(
						(t) => t.prUrl === prMetadata?.url,
					);
				...
				setPlatformReviewRecovery(
					recovery ? { rootPrUrl: prMetadata.url, recovery } : null,
				);
				...
				const persistsRefresh = recoveryStorage
					...
					: false;
				setPlatformRecoveryPersistsRefresh(
					recovery !== null && persistsRefresh,
				);
	...
	const openPlatformDialog = useCallback(
		(action: "approve" | "comment") => {
			...
			const inMemoryRecovery =
				rootPrUrl && platformReviewRecovery?.rootPrUrl === rootPrUrl
					? platformReviewRecovery.recovery
					: null;
			const recoveryStorage = getReviewRecoveryStorage();
			const storedRecovery =
				rootPrUrl && recoveryStorage
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (platformCommentDialog) {
				...
				const retryBlocked = platformCommentDialog.plan.targets.some(
					(target) => target.status === "blocked",
				);
				const canSubmit =
					isApproveAction || hasTargets || platformGeneralComment.trim();
		...
		<ThemeProvider defaultTheme="dark">
                        ...
                        shortcut routes to Approve, so this hides cleanly. */}
									...
									<button
										...
										className={`relative p-1.5 rounded-md transition-all ${
											reviewSidebar.isOpen &&
											reviewSidebar.activeTab === "annotations"
												? "bg-primary/15 text-primary"
												: "text-muted-foreground hover:text-foreground hover:bg-muted"
										}`}
										title="Annotations"
									...
									{agentJobs.capabilities?.available && (
										<button
											...
											className={`relative p-1.5 rounded-md transition-all ${
												reviewSidebar.isOpen &&
												reviewSidebar.activeTab === "agents"
													? "bg-primary/15 text-primary"
													: "text-muted-foreground hover:text-foreground hover:bg-muted"
											}`}
											title="Review Agents"
							...
							>
									...
									panelView === "sections" && (
										<div className="contents group/sidebar">
											<SectionsPanel
												...
												enableKeyboardNav={
													!showExportModal && hasSearchableFiles
												}
												annotations={allAnnotations}
												...
												detectedBase={
													gitContext?.defaultBranch ||
													gitContext?.compareTarget?.fallback
												}
												...
												searchInputRef={
													hasSearchableFiles ? searchInputRef : undefined
												}
												onOpenSearch={
													hasSearchableFiles ? openSearch : undefined
												}
												onSearchChange={
													hasSearchableFiles
														? handleSearchInputChange
												...
												onSearchClear={
													hasSearchableFiles ? clearSearch : undefined
												}
												onSearchClose={
													hasSearchableFiles ? closeSearch : undefined
												}
												searchGroups={hasSearchableFiles ? searchGroups : []}
												searchMatches={hasSearchableFiles ? searchMatches : []}
												activeSearchMatchId={
													hasSearchableFiles ? activeSearchMatchId : null
												}
												onSelectSearchMatch={
													hasSearchableFiles
														? handleSelectSearchMatch
												...
												onStepSearchMatch={
													hasSearchableFiles ? stepSearchMatch : undefined
												}
											/>
											<ResizeHandle
									...
									showCommitsPanel && (
										<div className="contents group/sidebar">
											<CommitsPanel
												...
												onExplainCommit={
													aiUIEnabled ? handleExplainCommit : undefined
												}
												explainingCommitSha={explainingCommitSha}
								...
								{!guideOpen &&
									shouldShowFileTree &&
									isFileTreeOpen &&
									!(sectionsAvailable && panelView === "sections") &&
									!showCommitsPanel && (
										<div className="contents group/sidebar">
											<FileTree
												files={files}
												...
												prOverviewNumber={
													prMetadata ? mrNumberLabel : undefined
												}
												prOverviewTitle={prMetadata?.title}
												onSelectPRArtifacts={
													prMetadata ? openPRArtifactsPanel : undefined
												}
												isPRArtifactsActive={isPRArtifactsActive}
												prArtifactCount={
													prMetadata ? prArtifacts.length : undefined
												}
												onSelectSemanticDiff={() => openSemanticDiffPanel()}
												isSemanticDiffActive={isSemanticDiffActive}
												semanticDiffAvailable={semanticDiffAvailable}
												...
												availableBranches={
													prMetadata ? undefined : gitContext?.availableBranches
												}
												selectedBase={
													prMetadata ? undefined : (selectedBase ?? undefined)
												}
												detectedBase={
													prMetadata
														? undefined
												...
												compareTarget={gitContext?.compareTarget}
												recentCommits={
													prMetadata ? undefined : gitContext?.recentCommits
												}
												jjEvologs={
													prMetadata ? undefined : gitContext?.jjEvologs
												}
												detectedEvoBase={
													prMetadata
														? undefined
												...
												onSwitchToSections={
													sectionsCapable ? handleSwitchToSections : undefined
												}
												onSwitchToCommits={
													commitsCapable
														? () => handlePanelViewSelect("commits")
														: undefined
												}
												sinceBaseSections={
													activeDiffBase === "since-base" ? sections : null
												}
              ...
              toggling the guide open and closed. */}
                ...
                see useCommitsView's veilActive. */}
													...
													{diffError ? (
														...
														<>
															...
															<p className="text-xs text-muted-foreground mt-1">
																{activeDiffBase === "since-base" &&
																	`No changes since ${selectedBase || gitContext?.defaultBranch || "main"}${activeWorktreePath ? " in this worktree" : ""} — committed, uncommitted, or untracked.`}
																{activeDiffBase.startsWith("commit:") &&
																...
																{activeDiffBase === "jj-last" &&
																	"No changes in the last jj change."}
																...
																{activeDiffBase === "jj-all" &&
																	"No files at the current jj change."}
																{activeDiffBase === "gitbutler:workspace" &&
																	"No applied GitButler workspace changes."}
																{activeDiffBase.startsWith(
																	"gitbutler:stack:",
																) &&
																	"No committed changes in this GitButler stack."}
																{activeDiffBase.startsWith(
																	"gitbutler:branch:",
																) &&
																	"No committed changes in this GitButler branch."}
																{activeDiffBase === "branch" &&
								...
								<ConfirmDialog
									...
									message={
										<div className="space-y-3">
											<p>
												This PR is checked out locally so review agents have
												full file access.
											</p>
											<div>
							...
							<ConfirmDialog
								...
								message={
									<>
										You have {totalAnnotationCount} annotation
										{totalAnnotationCount !== 1 ? "s" : ""} that will be lost if
										you approve.
									</>
								}
            ...
            doesn't render otherwise. */}
							...
							<ReviewSubmissionDialog
								...
								submission={
									platformCommentDialog?.plan ?? { targets: [], orphans: [] }
								}
						...
						{/* Tour dialog overlay */}
						<TourDialog
							jobId={tourDialogJobId}
							onClose={() => setTourDialogJobId(null)}
						/>
          ...
          Stripped from production builds via import.meta.env.DEV. */}
						{import.meta.env.DEV && (
							<button
								onClick={() =>
									setTourDialogJobId(
										tourDialogJobId === DEMO_TOUR_ID ? null : DEMO_TOUR_ID,
									)
								}
